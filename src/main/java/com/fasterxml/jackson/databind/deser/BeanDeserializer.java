    ...
    implements java.io.Serializable
{
    ...
    private static Logger log = Logger.getLogger(BeanDeserializer.class);
...
//        }
        JsonToken t = jp.getCurrentToken();
        StringBuffer propNameUnkown = new StringBuffer();
...
//				e.printStackTrace();
				propNameUnkown.append(propName + ",");
				continue;
			}
        }
        
        if(propNameUnkown != null && propNameUnkown.length() > 0) {

            log.error("处理controller参数自动填充值时出错!" +
                    "主要原因是前台传入的和相应的entity属性不匹配导导致(一般是多出一个enity中没有属性);具体信息如下:");
            log.error(
                    "\nbean:" + bean + ";\n" +
                            "unkown prop:" + propNameUnkown + "\n");
        }
