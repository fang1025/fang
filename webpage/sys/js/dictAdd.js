...
funcList.onload = function() {
	...
	currentData = fangjs.getSessionStorage("dictInfo");
	if(currentData){
		fangjs.showEntity(currentData, ['input', 'select', 'textarea']);
		fangjs.deleteSessionStorage("dictInfo");
		if( 'read' == act){
			$("select,input,textarea").attr("disabled",true);
			$(":button").remove();
		}else if('add' == act){
			$("#dictValue").attr("params","dictCode=" + currentData.dictCode);
		}
	}
...
funcList.saveDict = function() {
	...
	var url = $("#id").val()?"updateDict":"addDict";
    ...
    fangjs.execjava('/sys/dict/' + url, params, callback, false);
};
