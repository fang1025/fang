...
//        String flag = StringUtil.getUUID8();
        logger.info("开始执行方法" + getBridgedMethod().toString());
        Object[] args = getMethodArgumentValues(request, mavContainer, providedArgs);
        if(args.length > 0) {
            logger.info("传入的参数如下:");
            for (int i = 0; i < args.length; i++) {
                Map param = new HashMap();
                param.put("第" + (i + 1) + "个参数值如下:", args[i]);
                try {
                    if(args[i] instanceof String || args[i] instanceof Map || args[i] instanceof Collection)
                        logger.info(JSONObject.toJSONString(param));
                    else
                        logger.info(args[i]);
                }catch (Exception e) {
                    logger.info("第" + (i + 1) + "个参数值如下:" + args[i].toString());
                }
